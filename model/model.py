...
class ImgEmbMeta(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc1 = nn.Linear(16 * 5 * 5, 120) # 需要计算清楚
    ...
    def forward(self, x):  # x [batch,channel,h,w]
        ...
        x = self.pool(x)
        x = x.view(-1, 16 * 5 * 5) #这里改动conv需要计算维度
...
class TrajEmbedTeam(nn.Module):
    ...
    def forward(self, x, count_num):  # (batch * K) * 7,这个k不固定,这里的count_num在static中已经提取过，直接做切片[batch*k在外面用加法合并],不过一旦传递标量就gg,不能parallel
        ...
        for index, trajs in enumerate(all_trajs):
            batch_trajs[index],att_score = self.attention(trajs)  # K * hiddendim,k不固定
            batch_attn.append(att_score.cpu().tolist())# 不然存储的是tensor
        return batch_trajs.cuda(),batch_attn  # 这里存到batch里了，所以要加cuda
...
class LastEmb(nn.Module):
    ...
    def forward(self, h_trajs, h_count_num, a_trajs, a_count_num, home_o,home_d,away_o,away_d, static):
        ...
        """
        h_trajs_embed,h_batch_attn = self.h_seqtrajs(h_trajs, h_count_num)
        a_trajs_embed,a_batch_attn = self.a_seqtrajs(a_trajs, a_count_num)  # batch,hidden_dim
        ...
        return local, glob, h_batch_attn,a_batch_attn


if __name__ == '__main__':
    # 注意下面调试的时候Batch_Size设置为2，不然会出现split错误
    model = LastEmb()
    h_trajs = torch.LongTensor([[1, 2, 4, 5, 2, 1, 2], [4, 3, 2, 9, 1, 2, 3], [2, 2, 3, 4, 5, 6, 1]])
    h_count_num = [1, 2]
    a_trajs = torch.LongTensor([[1, 2, 4, 5, 2, 1, 2], [4, 3, 2, 9, 1, 2, 3], [2, 2, 3, 4, 5, 6, 1]])
    ...
    static = torch.randn(2, 16)
    local,glob,_,_ = model(h_trajs, h_count_num, a_trajs, a_count_num, home_o, home_d, away_o, away_d, static)
    print(local)
